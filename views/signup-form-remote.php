...
// NOTE: Remote placement requires completely different design because we may have to deal with all kind of different CSS?>
...
<form method="post" enctype="multipart/form-data" onsubmit="return validateBFTProUser(this,<?php echo $list->require_name?'true':'false'?>);" <?php if($remote_placement):?>action="<?php echo $form_action;?>"<?php endif;?> style="<?php echo $inline_style?>">
	...
	<fieldset>
		<?php $this->fields(@$list->id, null, null, true); ?>		
		
		<?php if(empty($list->id)):?>
			<div style="display: block; margin: 0.5rem auto;"><label style="display: block; margin: 0.5rem 0;"><?php _e('Mailing list:', 'bftpro')?></label>
		...
		<?php if(!empty($recaptcha_html)):?><div style="display: block; margin: 0.5rem auto;"><label>&nbsp;</label><?php echo $recaptcha_html?></div><?php endif;?>	
